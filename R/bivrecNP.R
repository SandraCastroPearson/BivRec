...
#'
#' @param object A response object of the \code{bivrecSurv} class.
...
#' @param x an object to be checked with is_bivrecNP
#'
#' @details
...
#' npresult <- bivrecNP(with(bivrec_data, bivrecSurv(id, epi, xij, yij, d1, d2)),
...
bivrecNP <- function(object, CI, ai, u1, u2, conditional, given.interval){
  x=object
  if (!is_bivrecSurv(x)) stop("Response must be a bivrecSurv class")
  if (missing(ai)) {ai<-1}
